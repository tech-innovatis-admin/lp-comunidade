...
export default function ConfettiEffect({ trigger, onComplete }: ConfettiEffectProps) {
  ...
  useEffect(() => {
    // Cria o manager apenas uma vez
    if (!managerRef.current) {
      managerRef.current = new ConfettiManager()
    }

    // Dispara o confete quando trigger muda de false para true
    if (trigger && !lastTriggerRef.current) {
      triggerCountRef.current += 1
      lastTriggerRef.current = true
      managerRef.current.resetAndStart()

      // Chama onComplete após um tempo (quando o confete terminar)
  ...
  return null // Não renderiza nada, o canvas é adicionado diretamente ao body
}
