...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsedFields = parseDocumentUploadFields(formData);
    if (!parsedFields.ok) {
      return jsonResponse(parsedFields.error, { status: 400 });
    }

    const { requirementCode } = parsedFields.value;
    const file = parsedFields.value.file as File;
